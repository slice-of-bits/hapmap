...
const defaultEatersStoreData: EatersStoreData = {
	eaters_total: 10,
	allergies: {}
};

function isAllergyCountMap(value: unknown): value is AllergyCountMap {
	return (
		typeof value === 'object' &&
		value !== null &&
		!Array.isArray(value) &&
		Object.values(value).every((count) => typeof count === 'number' && Number.isFinite(count))
	);
}

function isEatersStoreData(value: unknown): value is EatersStoreData {
	return (
		typeof value === 'object' &&
		value !== null &&
		!Array.isArray(value) &&
		'eaters_total' in value &&
		'allergies' in value &&
		typeof value.eaters_total === 'number' &&
		Number.isFinite(value.eaters_total) &&
		isAllergyCountMap(value.allergies)
	);
}

class EatersStore {
	private data: EatersStoreData = $persist(
		defaultEatersStoreData,
		'eaters',
		safePersistOptions(defaultEatersStoreData, isEatersStoreData)
	);
